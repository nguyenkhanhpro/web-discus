    ...
    <div class="content">
        ...
        <section>
            ...
            <?php
                ...
                {
                    $user_id = $_SESSION['user_id'];
                    $sql = "SELECT question.question_id, question.question, users.user_name, users.avatar,question.posting_time
                            FROM question
                    ...
                    while ($row = $result_name-> fetch_assoc())
                    {
                        ...
                        $sql_select_users_accs = "SELECT * FROM `users` 
                                                    INNER JOIN decentralization ON decentralization.decentralization_id = users.decentralization_id 
                                                    INNER JOIN question ON question.user_id = users.user_id
                                                    WHERE question.question_id = $question_id";
                        $users_accs= $conn -> query($sql_select_users_accs) -> fetch_assoc();
                        $users_acc = $users_accs["user_id"];
                        $dectra = $users_accs["decentralization_id"];
                        if($dectra == 1 or $users_acc == $user_id)
                        {
                        ...
                        while ($comments = $result_comment -> fetch_assoc())
                        {
                            ...
                            echo "<div class = 'comment_all' >
                                    ...
                                    if($dectra == 1 or $users_acc == $user_id )
                                    {
                                        echo "<input type=checkbox name = 'checkbox[]' value = '".$time_comment."' class='checkbox'>
                                        </form>";
                                    }
                            echo"</div>";
                        }
